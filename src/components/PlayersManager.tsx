...
import { UserPlus, Trash2, Trophy, BarChart, User, AlertCircle } from 'lucide-react';
...
export const PlayersManager: React.FC<PlayersManagerProps> = ({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (isDuplicate) {
      setErrorMessage(t.playersView.duplicateNameError);
      return;
    }
    setErrorMessage(null);
    onAddPlayer(trimmed);
          ...
          <div className="flex flex-col">
            <form onSubmit={handleSubmit} className="flex gap-2">
              <input
                  ...
                  className={`px-4 py-2 bg-slate-950 border rounded-xl text-sm text-white placeholder-slate-500 focus:outline-none w-full sm:w-64 transition-colors ${
                      errorMessage
              ...
              <button
                  ...
                  className="px-4 py-2 bg-amber-500 hover:bg-amber-400 disabled:opacity-50 text-slate-950 font-bold text-sm rounded-xl transition-colors flex items-center gap-1.5 shrink-0 shadow-sm cursor-pointer disabled:cursor-not-allowed"
              >
