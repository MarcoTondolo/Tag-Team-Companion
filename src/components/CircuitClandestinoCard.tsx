...
interface CircuitClandestinoCardProps {
  ...
  onWaveChange?: (currentWave: number) => void;
  onEndMatch?: (finalWave: number) => void;
}
...
export const CircuitClandestinoCard: React.FC<CircuitClandestinoCardProps> = ({
  ...
  // Circuit Run State
  const [wave, setWave] = useState<number>(1); // 1, 2, 3... infinite
  const [momentum, setMomentum] = useState<number>(0);

  useEffect(() => {
    if (onWaveChange) onWaveChange(wave);
  }, [wave, onWaveChange]);
  ...
  // Complete Wave & Apply Recovery (Infinite Waves)
  const handleCompleteWave = () => {
    ...
    setLogHistory((prev) => [
      isIt
          ? `🎉 Ondata ${wave} Superata! Recupero applicato (+${hpBonus} HP, +${powerBonus} Potere). Inizio Ondata ${nextWave}!`
          : `🎉 Wave ${wave} Cleared! Recovery applied (+${hpBonus} HP, +${powerBonus} Power). Starting Wave ${nextWave}!`,
      ...prev.slice(0, 4),
    ]);
  };
        ...
        <div className="bg-slate-950/90 border border-amber-500/30 rounded-2xl p-4 space-y-3 shadow-inner">
          ...
          {!isWaveCleared ? (
              ...
              >
                ...
                {isIt
                    ? `Ondata ${wave} Azzerata! Passa all'Ondata ${wave + 1} (+Recupero HP)`
                    : `Wave ${wave} Cleared! Advance to Wave ${wave + 1} (+HP Recovery)`}
              </button>
          )}
        </div>
